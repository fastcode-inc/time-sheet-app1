    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("timesheet")) {
                if (
                    ...
                    builder.and(
                        timesheetdetails.timesheet.periodstartingdate.eq(
                    ...
                    builder.and(
                        timesheetdetails.timesheet.periodstartingdate.ne(
                            SearchUtils.stringToLocalDate(details.getValue().getSearchValue())
                        )
                    );
                } else if (details.getValue().getOperator().equals("range")) {
                    ...
                    if (startLocalDate != null && endLocalDate != null) {
                        builder.and(
                            timesheetdetails.timesheet.periodstartingdate.between(startLocalDate, endLocalDate)
                        );
                    } else if (endLocalDate != null) {
        ...
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            ...
            if (joinCol != null && joinCol.getKey().equals("timesheet")) {
                builder.and(
                    timesheetdetails.timesheet.periodstartingdate.eq(SearchUtils.stringToLocalDate(joinCol.getValue()))
                );
            }
        }
        return builder;
